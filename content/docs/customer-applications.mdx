...
for facts valid when the call begins. It is not a place for current balance,
case history, credentials, or authorization tokens.
...
3. Stable explanation of your payment policy can live in a Knowledge Base.
   Current balance, due date, account status, and payment link must be requested
   through a narrow read Action, not copied into the prompt or `call_context`.
