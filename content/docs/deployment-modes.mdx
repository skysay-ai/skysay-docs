...
deployment-bound `skygw_` token. Create it in the **Gateway Tokens** card after
the gateway deployment is registered. Its one-time reveal includes this exact
line; replace the blank `SKYSAY_GATEWAY_API_TOKEN=` assignment in the
...
- `tools/list` is the only MCP method a gateway token may call. It returns the
  ...
  resource methods stay refused, so a stolen `skygw_` cannot send an SMS or read
...
Set `SKYSAY_ORGANIZATION_ID` to the workspace that registered the gateway
...
```bash
SKYSAY_CONTROL_PLANE_URL=https://api.skysay.ai \
SKYSAY_BOOTSTRAP_API_KEY=sky_... \
  bash selfhost-bootstrap.sh
```

It registers the deployment, mints the `skygw_` token (with no expiry — an expiry
...
| Credentials on the host | the `skygw_` token only | the `skygw_` token **and** an organization-scoped `sky_` key |
...
`POST /v1/gateway-deployments/{gateway_id}/outbound-calls` with its own `skygw_`
...
`SKYSAY_WORKER_API_TOKEN` is an organization-scoped `sky_` key. Inbound
