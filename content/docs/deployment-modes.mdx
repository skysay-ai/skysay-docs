...
## Staged shared hosted execution

The platform simulation runner and outbound gateway/dialer implementations are
preparation for a coordinated release, not confirmation that shared hosted
execution is live. Hosted work derives its workspace, agent and caller number
from the claimed job and canonical call. A new restricted gateway credential
authorizes that existing call under its current worker lease; it does not turn
a customer gateway into a cross-workspace gateway. Existing customer credentials
and self-hosted queues remain organization-bound.

There is no per-workspace hosted gateway provisioning step. A self-hosted LOCAL
gateway dials its own calls; a customer-operated queue keeps its own jobs. Other
hosted work is routed to the shared workers after the controlled migration.
A stopped hosted worker can still leave work queued: shared execution removes
workspace-specific provisioning, not the need to monitor worker availability.

Customers do not set up these platform roles or change host environment values.
Activation follows the complete implementation release, controlled credential
cutover and production verification. See the
[staged authentication contract](/docs/authentication#staged-hosted-worker-contract).
