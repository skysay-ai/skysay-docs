...
the shipped stack that still reads it is the optional outbound-dial profile,
because claiming outbound call jobs is `worker_jobs:*` authority over the whole
workspace queue that a deployment-bound token deliberately does not carry.

An inbound-only host therefore holds no organization-wide credential at all.
