...
Every REST route that names a resource by id in its path answers **`404`** when
that id belongs to another workspace — the same status and the same body it
answers for an id that never existed at all. The two cases are deliberately
indistinguishable, so a status code can never be used to discover which ids are
real in someone else's workspace.

Treat `404` on these routes as *"not yours, or not there"*. Do not branch on
`403` versus `404` to decide whether a record exists: a `403` on a by-id route
now means something about your **credential** — a missing scope, or a request
that named an organization other than the key's — and never something about the
record.

Two answers are unchanged, because neither depends on the id:
...
`GET /v1/organizations/{organization_id}/...` is the one shape where the path id
