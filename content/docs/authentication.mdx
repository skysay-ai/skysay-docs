...
description: Scoped sky_ keys and gateway skygw_ tokens, Bearer auth, scope clamping and fail-closed semantics, org isolation, what the API expects and answers, and rotate/revoke.
icon: Key
---
...
curl -s -X POST "$SKYSAY_BASE_URL/v1/api-keys" \
  -H "Authorization: Bearer $SKYSAY_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Read-only reporting key",
       "scopes":["all-read"]}'
```
...
### `organization_id` is optional with a workspace key

A workspace key belongs to exactly one workspace, so you never need to name
it. Wherever a REST route accepts `organization_id`, as a query parameter or a
request-body field, you can leave it out and the key's own workspace is used:

```bash
curl -s "$SKYSAY_BASE_URL/v1/calls?limit=20" \
  -H "Authorization: Bearer $SKYSAY_API_KEY"
```

If you do send it, it must be the key's own workspace. Naming any other
workspace answers `403`, whether or not that workspace exists. `project_id` is
different: it is never filled in for you, and a route that requires it, such
as `POST /v1/agents`, answers `400` without it. The
[Quickstart with curl](/docs/quickstart-curl) sends no `organization_id` at all.
...
## What the API expects a request body to be

Every write you make takes a UTF-8 JSON **object**. (Provider callback routes
that have always accepted form-encoded or plain-text bodies still do; those are
URLs you give a carrier, not endpoints you call.) Four answers follow, and none
of them is a `500`:

- A body that is absent, or contains only whitespace, is read as an empty
  object. Action routes that take no fields can keep sending nothing.
- A body that is not valid JSON, is not UTF-8, or is valid JSON that is not an
  object — an array, a number, a bare string — answers `400`.
- A request that carries a `Content-Encoding` other than `identity` answers
  `415`. Skysay does not accept a compressed request body; send it
  uncompressed. Response compression is unaffected, and so is
  `Accept-Encoding`.
- A required field that is missing, `null`, or blank answers `400` naming the
  field, rather than failing further in. `POST /v1/agents` and
  `POST /v1/numbers` require `project_id` this way;
  `POST /v1/agent-endpoints` also requires `webhook_url` and
  `audio_stream_url`. `organization_id` is never required from a workspace
  key; see [`organization_id` is optional](#organization_id-is-optional-with-a-workspace-key).

If you are retrying a request because it returned `500`, check it against this
list first: several shapes that used to answer `500` now answer `400` or `415`,
and those are answers about the request, not about the service.

## When Skysay itself fails

A `4xx` tells you what was wrong with your request, and its `message` is
written for you to read:

```json
{
  "error": "BadRequestError",
  "message": "mode must be external_audio, external_text, or hosted"
}
```

A `5xx` that Skysay did not classify is different: nothing about it was
written for you. It answers one fixed shape on every route, with a
`request_id` that identifies this one failure.
...
- `error` is always the literal `internal_error`. Branch on it if you retry
  server failures; it will not change into something else.
- `message` is always that same sentence. It carries no detail about the
  failure, deliberately — the failing request's own text stays on our side,
  where it cannot hand you a filesystem path, a provider's reply, or an
  identifier belonging to another workspace.
- `request_id` is unique to that one request. Quote it to support and we can
  find the failure; keep it in your own logs beside the request that failed.

Some `5xx` answers are not this shape, because they are designed rather than
unexpected — `503` from a webhook endpoint asking a provider to redeliver, or
a `502` naming a verification provider's own status. Those keep their specific
`error` code and carry no `request_id`.
