...
- A required field that is missing, `null`, or blank answers `400` naming the
  ...
  `POST /v1/numbers` require `organization_id` and `project_id` this way;
...
```json
{
  ...
  "request_id": "req_Ab3xYz7QmNp2Kd9R"
}
```
...
- `message` is always that same sentence. It carries no detail about the
  failure, deliberately — the failing request's own text stays in our logs,
