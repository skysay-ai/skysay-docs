...
The hosted outbound implementation separately uses a newly operator-issued
gateway credential with only `calls:authorize` and `gateways:report`. Its explicit
platform role authorizes an existing hosted call using that call's current
worker lease and tenant-bound configuration; it cannot enqueue jobs or fetch
arbitrary agent configuration. The gateway retains its platform owner's
organization. Changing its deployment mode never upgrades existing tokens.
Revoking a platform-role gateway credential also requires an operator dashboard
session; an organization key cannot administer that shared role.
Both platform consumers and the controlled cutover must
...
The controlled migration preserves genuine customer queues and retires only
explicitly inventoried old hosted identities. Legacy queued simulations receive
verified call/project/agent bindings before hosted classification; malformed
source relationships are quarantined without changing another workspace's call
or wallet. Running or previously attempted hosted work requires reconciliation,
not automatic replay. None of this provisions a hosted gateway per customer.

New gateway tokens default to the two originator scopes above. The self-hosted
bootstrap and workspace gateway action explicitly request their full runtime
scope set, including number rules, inbound enqueue, artifact upload and component
credential leases. Existing tokens retain their stored scopes; custom bootstrap
clients must request every runtime permission they need.
