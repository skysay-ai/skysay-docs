...
| `get_agent_catalog` | `agents:read` **or** `coverage:read` | Selectable STT/LLM/TTS providers with cost, latency, languages, streaming, runnability; optional `metered_only: boolean` defaults to `false`, and `true` returns only component-metered choices. |
...
| `get_outbound_permit` | `onboarding:read` | Read the workspace outbound permit (caps, gates). |
| `request_outbound_permit` | `onboarding:write` | Request an outbound-permit change. `countries` is required and must name at least one ISO 3166-1 alpha-2 destination. |
